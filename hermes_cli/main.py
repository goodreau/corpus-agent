...
Usage:
    ...
    hermes uninstall           Uninstall Corpus Agent
...
def _print_fast_version_info() -> None:
    ...
    print(f"Corpus Agent v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Corpus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_uninstall(args):
    """Uninstall Corpus Agent (or just the Chat GUI with --gui)."""
...
def _update_via_zip(args):
    """Update Corpus Agent by downloading a ZIP archive.
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Corpus Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    """Update Corpus Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Corpus Agent")
        return
...
def cmd_acp(args):
    """Launch Corpus Agent as an ACP server."""
    try:
