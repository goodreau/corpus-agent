...
class CLICommandsMixin:
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Corpus Agent to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Corpus Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Corpus Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Corpus Agent",
