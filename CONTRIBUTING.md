# Contributing to Corpus Agent

Thank you for contributing to Corpus Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Corpus Agent to work everywhere.
...
Related: #38284 covers the agent-side analog — Corpus Agent itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Corpus Agent install. They should be **broadly useful to most users**:
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Corpus Agent ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.hermes/plugins/`, project `.hermes/plugins/`, or a pip entry point) — see [Build a Corpus Agent Plugin](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin)
...
The installer creates the Corpus Agent venv, wires the `hermes` command, stamps the
...
Use this only if you intentionally do not want Corpus Agent's managed install layout
...
The user may skip setup and keep loading the skill. Corpus Agent only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
2. **Tools referenced in SKILL.md prose must be native Corpus Agent tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Corpus Agent" is the secondary collaborator. If the contributor's commit shows "Corpus Agent" as author because they used Corpus Agent to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Corpus Agent tools (`web_extract`, `terminal`, `read_file`).
...
Corpus Agent uses a data-driven skin system — no code changes needed to add a new skin.
...
Corpus Agent runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Corpus Agent has terminal access. Security matters.
...
- Include: OS, Python version, Corpus Agent version (`hermes version`), full error traceback
