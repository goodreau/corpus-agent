...
sidebar_position: 3
title: '学习路径'
description: '根据您的经验水平和目标，选择适合您的 Corpus Agent 文档学习路径。'
---

# 学习路径

Corpus Agent 功能丰富——CLI 助手、Telegram/Discord 机器人、任务自动化、强化学习训练等。本页帮助您根据自身经验水平和目标，确定从哪里开始、阅读哪些内容。

:::tip 从这里开始
如果您尚未安装 Corpus Agent，请先阅读[安装指南](/getting-started/installation)，然后完成[快速入门](/getting-started/quickstart)。以下内容均假设您已完成安装。
:::
...
- **随便浏览？** 查看[主要功能](#key-features-at-a-glance)表格，快速了解 Corpus Agent 的全部能力。
...
将 Corpus Agent 用作交互式终端助手，用于编写、审查和运行代码。
...
:::tip
通过上下文文件将文件直接传入对话。Corpus Agent 可以读取、编辑并运行您项目中的代码。
:::

### "我想要一个 Telegram/Discord 机器人"

将 Corpus Agent 部署为您常用消息平台上的机器人。
...
:::tip
Cron 任务让 Corpus Agent 按计划执行任务——每日摘要、定期检查、自动报告——无需您在场。
:::

### "我想构建自定义工具/技能"

通过自定义工具和可复用技能包扩展 Corpus Agent。
...
使用强化学习（RL）通过 Corpus Agent 内置的 RL 训练流水线对模型行为进行微调。
...
:::tip
强化学习训练在您已了解 Corpus Agent 如何处理对话和工具调用的基础上效果最佳。如果您是新手，请先完成初级路径。
:::

### "我想将其作为 Python 库使用"

以编程方式将 Corpus Agent 集成到您自己的 Python 应用中。
