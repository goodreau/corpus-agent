...
The ACP adapter wraps Corpus' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- `allow_once` -> Corpus `once`
- `allow_always` -> Corpus `always`
- reject options -> Corpus `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Corpus tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Corpus' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Corpus provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run registry clients can open Corpus' interactive model/provider configuration before starting a normal ACP session.
