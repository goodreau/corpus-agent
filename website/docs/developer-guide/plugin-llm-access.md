...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Corpus already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Corpus provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Corpus provider
        # (must be one Corpus already knows about — same names as
        ...
        # plugin sends — Corpus does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Corpus' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Corpus subsystem:
