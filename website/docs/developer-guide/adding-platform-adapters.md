...
This guide covers adding a new messaging platform to the Corpus gateway. A platform adapter connects Corpus to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core Corpus code. Your plugin is a directory with two files:
...
def register(ctx):
    """Plugin entry point — called by the Corpus plugin system."""
...
version: 1.0.0
description: >
  My Platform gateway adapter for Corpus Agent.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the Corpus core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
