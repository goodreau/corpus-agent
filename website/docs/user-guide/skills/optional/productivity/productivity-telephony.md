---
title: "Telephony — Give Corpus phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Corpus phone capabilities without core tool changes"
---
...
Give Corpus phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
...
This optional skill gives Corpus practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn Corpus into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Corpus memory
...
### 1) "I want Corpus to own a real phone number"
Use **Twilio**.
...
- pairs well with Corpus `text_to_speech` plus a public file host or tunnel
...
Then save credentials into Corpus:
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Corpus calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Corpus's existing `text_to_speech` support.

Use this when:
- you want the call to use Corpus's configured TTS voice rather than Twilio `<Say>`
...
Recommended Corpus TTS -> Twilio Play workflow:

1. Generate the audio with Corpus `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Corpus to create the message audio with `text_to_speech`
...
Important note:
- Corpus TTS is great for prerecorded outbound messages
...
- Corpus STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Corpus memory.
...
- Do not store arbitrary third-party phone numbers in Corpus memory.
