...
This is an optional third-party workflow, not a Corpus core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Corpus native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Corpus native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Corpus native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Corpus usage patterns
...
- Do not silently choose Parallel when Corpus native tools are already sufficient.
