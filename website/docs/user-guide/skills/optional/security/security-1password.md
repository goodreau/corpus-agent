...
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Corpus Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Corpus terminal calls (desktop app flow only)
...
### Service Account (recommended for Corpus)
...
## Corpus Execution Pattern (desktop app flow)

Corpus terminal commands are non-interactive by default and can lose auth context between calls.
