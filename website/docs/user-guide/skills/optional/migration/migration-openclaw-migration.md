---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Corpus Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Corpus Agent"
---
...
Migrate a user's OpenClaw customization footprint into Corpus Agent. Imports Corpus-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | Corpus Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Corpus`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Corpus Migration

Use this skill when a user wants to move their OpenClaw setup into Corpus Agent with minimal manual cleanup.
...
- import `SOUL.md` into the Corpus home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Corpus memory entries
- merge OpenClaw command approval patterns into Corpus `command_allowlist`
- migrate Corpus-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Corpus working-directory configuration
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Corpus workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Corpus destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Corpus-compatible secrets, currently:
...
Corpus CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Corpus. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Corpus workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Corpus skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Corpus unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Corpus targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Corpus destination. Unsupported auth blobs must still be reported as skipped.
...
- Corpus persona state imported
- Corpus memory files populated with converted OpenClaw knowledge
