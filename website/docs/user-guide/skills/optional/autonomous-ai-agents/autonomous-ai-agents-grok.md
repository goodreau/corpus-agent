...
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), Corpus Agent |
| License | MIT |
...
# Grok Build CLI — Corpus Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Corpus terminal. Grok
...
1. Prepare stable input files first with Corpus tools (`read_file`,
...
2. **Don't conflate Corpus' xAI auth with the `grok` CLI's auth.** Corpus'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Corpus' xAI auth carries
   over.
