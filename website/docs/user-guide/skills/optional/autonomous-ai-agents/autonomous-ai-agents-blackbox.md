...
Delegate coding tasks to [Blackbox AI](https://www.blackbox.ai/) via the Corpus terminal. Blackbox is a multi-model coding agent CLI that dispatches tasks to multiple LLMs (Claude, Codex, Gemini, Blackbox Pro) and uses a judge to select the best implementation.
