...
| Version | `0.1.0` |
| Author | Tim Koepsel (xzessmedia), Corpus Agent |
| License | MIT |
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Corpus.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Corpus-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hermes-agent` (Corpus subagents via `delegate_task`).
