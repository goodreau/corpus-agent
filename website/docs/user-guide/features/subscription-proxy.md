...
chat completions — use your Corpus-managed provider subscription as their
...
| Use case | "Use Corpus as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Corpus stores
the refresh token in `~/.hermes/auth.json` — the same place all Corpus
...
```
Starting Corpus proxy for Nous Portal
...
```
Corpus proxy upstream adapters
