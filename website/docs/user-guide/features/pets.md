...
Corpus can show an animated **pet** — a small mascot sprite that reacts to what
...
In the terminal (CLI/TUI), Corpus renders the sprite at full fidelity when your
...
Beyond installing pre-made pets from the gallery, Corpus can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.
...
always-on-top desktop window. Out there it stays visible while Corpus is
