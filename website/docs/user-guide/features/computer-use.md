...
Corpus Agent can drive your desktop — clicking, typing, scrolling,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Corpus app). `hermes computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each Corpus run declares its own cua-driver
...
Corpus intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Corpus-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Corpus skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Corpus
...
Corpus applies multi-layer guardrails:
...
Screenshots are expensive. Corpus applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Corpus agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Corpus agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Corpus warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Corpus from inside the RDP / console
...
upstream. **Corpus disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Corpus sets
...
unreleased fix — point Corpus at a binary you built from source instead
of the published release. Corpus resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Corpus testing (see notes).
...
### Option B — point Corpus straight at the built binary (fastest loop)
...
### Confirm Corpus is using your build
...
- **Corpus spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Corpus
  change needed either way.
- **Local builds skip the version check.** Corpus warns when the
...
- **Corpus-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Corpus `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Corpus (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Corpus among them)
