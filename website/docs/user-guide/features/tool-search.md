...
**Tool Search** is Corpus' opt-in progressive-disclosure layer for that
...
:::info Built-in Corpus tools never defer
The tools that make up Corpus' core capability set (`terminal`,
...
When the model invokes `tool_call`, Corpus **unwraps the bridge** and
...
- **No JS sandbox.** Corpus uses the simpler "structured tools" mode
