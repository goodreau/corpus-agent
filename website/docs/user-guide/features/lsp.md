...
Corpus runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Corpus
...
1. Corpus captures a baseline of current diagnostics for the file.
...
…). Corpus auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Corpus at the extracted bundle — the directory that contains
...
`node_modules` tree — Corpus installs both packages together when you
...
When `install_strategy: auto`, Corpus installs binaries into
...
shared location — the staging dir is fully Corpus-owned and is
...
Servers are kept alive for the life of the Corpus process. There's
