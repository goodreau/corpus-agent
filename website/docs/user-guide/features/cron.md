...
Schedule tasks to run automatically with natural language or cron expressions. Corpus exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
Corpus will use the unified `cronjob` tool internally.
...
The `cronjob` tool's schema exposes `no_agent` to Corpus directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Corpus will write the check script to `~/.hermes/scripts/` via `write_file`, then call:
...
- When Job 2 fires, Corpus reads Job 1's most recent output from `~/.hermes/cron/output/{job1_id}/*.md`
...
When `enabled_toolsets` is set on a job it wins; otherwise the `hermes tools` cron-platform config wins; otherwise Corpus falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Corpus should even invoke the agent. Emit a final stdout line of the form:
...
:::tip
Corpus's own `~/.hermes/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::
...
Jobs may store `model` and `provider` as `null`. When those fields are omitted, Corpus resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
