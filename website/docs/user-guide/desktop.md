...
sidebar_position: 3
title: "Desktop App"
description: "The native Corpus desktop app — a polished experience for chatting with Corpus, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Corpus desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Corpus Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `hermes` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Corpus has several front ends that all talk to the same agent:
...
Follow the [installation instructions for Corpus Desktop](../getting-started/installation.md).

If you already have Corpus installed, simply run
...
- **The same conversation history** as every other Corpus surface — sessions started here resume in the CLI/TUI and vice versa.
...
Talk to Corpus and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
The app also surfaces the broader Corpus management surface so you don't have to drop to a terminal:
...
- **Profiles** — switch between [Corpus profiles](./profiles.md) (isolated config/skills/sessions).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Corpus agent, your config, and your chats stay. (Same as `hermes uninstall --gui`.)
...
| `--hermes-root PATH` | Override the Corpus source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Corpus Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Corpus Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).
...
rm "$HOME/.hermes/@@PROT56@@/.hermes-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.hermes/@@PROT57@@/venv"
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.hermes/@@PROT58@@/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
