...
Connect Corpus to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Corpus — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Corpus runs it in a
...
2. **Finds or creates** the `Corpus Agent` project on your account.
...
Photon uses the same authorization model as every other Corpus
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Corpus replies with a pairing code. Approve it with:
...
By default Corpus responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Corpus` and
`@Corpus agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Corpus will reply.
...
assigned iMessage line Corpus uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Corpus sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Corpus wake words  | JSON list / comma / newline regex patterns for group mentions |
