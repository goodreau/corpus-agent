...
sidebar_position: 1
title: "Telegram"
description: "Set up Corpus Agent as a Telegram bot"
---

# Telegram Setup

Corpus Agent integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "Corpus Agent") — this can be anything
...
Enable `status_indicator` and Corpus sets that short description to **Online**
...
`priority_mode` controls how your list combines with Corpus' built-in priority list:

- `prepend`: put your commands first, then Corpus defaults
- `append`: keep Corpus defaults first, then your commands
- `replace`: use only your list for priority ordering

Telegram allows up to 100 BotCommands, but large command payloads can fail. Corpus defaults to 60 for reliability and clamps configured values to `1..100`; use `/commands` for the full command list.
...
Corpus Agent uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
By default, Corpus connects to Telegram using **long polling** — the gateway makes outbound requests to Telegram's servers to fetch new updates. This works well for local and always-on deployments.
...
Voice messages you send on Telegram are automatically transcribed by Corpus's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running Corpus — no API key required
...
With STT disabled, the gateway still downloads the voice/audio attachment into Corpus's audio cache, but **does not transcribe it**. The agent receives the message with a marker like:
...
Telegram's **public** Bot API caps `getFile` downloads at **20 MB**, so any voice note, audio file, video, or document larger than that is silently rejected by Corpus with a "too large" reply. The documented way around this is to run a **local** [telegram-bot-api](https://github.com/tdlib/telegram-bot-api) daemon — the same server software Telegram uses, but running on your network. A local server raises the file ceiling to **2 GB** and Corpus auto-lifts its own internal cap when it sees a custom `base_url` configured.
...
### Step 4: Point Corpus at the local server
  ...
  telegram:
    extra:
      ...
      local_mode: true        # see Step 5 below — only set this if the bot's data
                              # directory is readable by the Corpus process
```
...
2. **With `--local`** (set via `TELEGRAM_LOCAL=1` above): files are written to the server's filesystem and the `getFile` response returns an **absolute path** instead of an HTTP URL. The 20MB ceiling is lifted. Corpus must then read the bytes **from disk**, not over HTTP.

To make the disk-read path work, set `local_mode: true` in the config above **and** make sure the Corpus process can read the path the server returns. Two scenarios:

- **Same machine** — telegram-bot-api and Corpus run on the same host. Bind-mount the data volume to a directory that Corpus can read (e.g., `/var/lib/telegram-bot-api`), and make sure the file ownership matches. The container drops privileges to its internal `telegram-bot-api` user (uid varies by image); the simplest fix is to add `user: "<UID>:<GID>"` to the compose service so files are owned by a uid Corpus already runs as.
- **Different machines** — the bot server runs on one host (e.g., a NAS, a separate VM) and Corpus on another. The server's data directory must be shared with the Corpus machine at the **same absolute path** the server reports (typically `/var/lib/telegram-bot-api`). NFS works well for this; CIFS/SMB with `uid=` mount remapping is friendlier if you don't want to deal with uid mismatches at the filesystem level.

If `local_mode: true` is set but Corpus can't `stat` the returned file path (permissions or wrong mount), python-telegram-bot silently falls back to an HTTP `getFile` against the local server — which in `--local` mode responds with `404 Not Found`. The symptom shows up in `gateway.log` as:
...
If you see that, the cap-lift is working but the file-share isn't. Verify `ls -la /var/lib/telegram-bot-api/<TOKEN>/voice/` from the Corpus host as the user the gateway runs as, and confirm a single file is `cat`-able without a permission error.
...
Corpus Agent works in Telegram group chats with a few considerations:
...
- In groups with multiple Corpus bots, `telegram.exclusive_bot_mentions` keeps routing deterministic. When a message explicitly mentions one or more Telegram bot usernames, only the mentioned bot profiles process it; other Corpus bots ignore it before reply and wake-word fallbacks run. This is enabled by default.
- Use `telegram.ignored_threads` to keep Corpus silent in specific Telegram forum topics, even when the group would otherwise allow free responses or mention-triggered replies
- If `telegram.require_mention` is left unset or false, Corpus keeps the previous open-group behavior and responds to normal group messages it can see

### Multiple Corpus bots in one group

If you run several Corpus profiles in the same Telegram group, create one Telegram bot token per profile and start one gateway per profile. Do not reuse the same bot token in multiple running gateways; Telegram will reject concurrent polling for the same token.
...
With this setup, a group message like `@research_bot @ops_bot summarize this` is processed by `research_bot` and `ops_bot` only. Other Corpus bots in the group stay silent, even if the message is a reply to one of their earlier messages or would otherwise match a shared wake word.
...
1. **Telegram delivery:** turn off BotFather privacy mode, promote the bot to
   admin, or mention the bot directly. Corpus cannot respond to group messages
...
3. **Corpus authorization:** make sure the sender is listed in
...
5. **Multi-bot routing:** if a group contains several bots, make sure each
   Corpus profile uses a unique bot token and keep `exclusive_bot_mentions`
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** — bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with Corpus.
...
1. Open your private chat with the Corpus bot in Telegram
2. Tap the bot's name at the top to open chat info
3. Enable **Topics** (the toggle to turn the chat into a forum)

Without this, Corpus will log `The chat is not a forum` on startup and skip topic creation. This is a Telegram client-side setting — the bot cannot enable it programmatically.
:::
...
1. On gateway startup, Corpus calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
...
A ChatGPT-style multi-session DM — one bot, many parallel conversations. Unlike the operator-curated `extra.dm_topics` above, this mode is **user-driven**: no config, no pre-declared topic names. The end user flips it on with `/topic`, then taps the Telegram **+** button to create as many topics as they want, each one a fully independent Corpus session.
...
| Topic names | Chosen by operator | Chosen by user; auto-renamed to match Corpus session title |
...
When the user first runs `/topic`, Corpus calls `getMe` to verify both flags. If either is off, Corpus sends a screenshot of the BotFather Threads Settings page and explains what to toggle — no activation happens until prerequisites are met.
...
4. Corpus responds inside that topic — the topic is now a standalone session
...
When Corpus generates a session title for a topic (via the auto-title pipeline, after the first exchange), the Telegram topic itself is renamed to match — e.g. "New Topic" becomes "Database migration plan". The rename is best-effort: failures are logged but don't break the session.
...
When this flag is on, Corpus still generates an internal session title (used by `hermes sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.

### `/new` inside a topic

Resets the current topic's session (new session ID, fresh history) without touching other topics. Corpus replies with a reminder that for parallel work, creating another topic (via **All Messages**) is usually what you want.
...
This binds the current topic to an existing Corpus session instead of starting fresh. Useful for continuing a conversation that started before topic mode was enabled. Restrictions:

- The target session must belong to the same Telegram user
- The target session must not already be bound to another topic

Corpus confirms with the session title and replays the last assistant message for context.

To discover session IDs, send `/topic` (no argument) in the root DM — Corpus lists the user's unlinked Telegram sessions.
...
Send `/topic off` in the root DM. Corpus flips the row off, clears the chat's `(thread_id → session_id)` bindings, and the root DM reverts to a normal Corpus chat. Existing topics in Telegram aren't deleted — they just stop being gated as independent sessions. Re-run `/topic` later to turn it back on.
...
1. When a message arrives in a mapped group topic, Corpus looks up the `chat_id` and `thread_id` in `group_topics` config
...
| Topic creation | Corpus creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
- **Bot API 9.4 (Feb 2026):** Private Chat Topics — bots can create forum topics in 1-on-1 DM chats via `createForumTopic`. Corpus uses this for two distinct features: operator-curated [Private Chat Topics](#private-chat-topics-bot-api-94) (config-driven, fixed topic list) and user-driven [Multi-session DM mode](#multi-session-dm-mode-topic) (activated by `/topic`, unlimited user-created topics).
- **Privacy policy:** Telegram now requires bots to have a privacy policy. Set one via BotFather with `/setprivacy_policy`, or Telegram may auto-generate a placeholder. This is particularly important if your bot is public-facing.
- **Bot API 9.5 (Mar 2026): Native streaming via `sendMessageDraft`.** Corpus supports Telegram's native streaming-draft API as an opt-in transport for private chats. The default remains the legacy `editMessageText` path because draft previews can visibly collapse and re-render on some Telegram clients.

### Streaming transport (`gateway.streaming.transport`)

When streaming is enabled (`gateway.streaming.enabled: true`), Corpus picks one of four transports:
...
**MarkdownV2 fallback.** When the rich path is unavailable for a message, Corpus converts markdown to MarkdownV2. Since MarkdownV2 has no native table syntax, pipe tables are normalized:
...
When enabled, Corpus attaches Telegram's `LinkPreviewOptions(is_disabled=True)` to every outgoing message and falls back to the legacy `disable_web_page_preview` parameter on older `python-telegram-bot` versions.
...
When you send `/model` with no arguments in a Telegram chat, Corpus shows an interactive inline keyboard for switching models:
...
The proxy applies to both the primary transport and all fallback IP transports. No additional Corpus configuration is needed — if the environment variable is set, it's used automatically.

:::note
This covers the custom fallback transport layer that Corpus uses for Telegram connections. The standard `httpx` client used elsewhere already respects proxy env vars natively.
:::
