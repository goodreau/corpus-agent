...
sidebar_position: 12
title: "Google Chat"
description: "Set up Corpus Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Corpus Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Corpus
...
download the file. Save it somewhere only Corpus can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — Corpus calls
...
- **App name**: whatever you want users to see ("Corpus" is reasonable).
...
`ADDED_TO_SPACE` event that Corpus uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Corpus
...
Install the dependencies the Google Chat adapter needs (no Corpus extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Corpus is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Corpus detects the
...
3. Download the JSON. Move it onto the host that runs Corpus.
4. Register the client with Corpus (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's Corpus home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Corpus isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Corpus will only attach the SA bearer
