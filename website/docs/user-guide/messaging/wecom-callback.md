...
Connect Corpus to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Corpus supports two WeCom integration modes:
...
3. Corpus decrypts the message, queues it for the agent
...
console. A mismatch between the token configured in Corpus and the token the
