...
sidebar_position: 4
title: "Slack"
description: "Set up Corpus Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Corpus Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Corpus instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Corpus uses the modern
...
The fastest path is to paste a manifest Corpus generates for you. It
...
### Option A: From a Corpus-generated manifest (recommended)
...
4. Enter an app name (e.g., "Corpus Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Corpus can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Corpus configuration issue.
:::
...
Corpus uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
Every Corpus command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Corpus command with its description.

Under the hood: Corpus ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/@@PROT0@@/blob/main/hermes_cli/commands.py)
...
When Corpus adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes btw run the tests` — Corpus routes it the same way as `/btw
...
Slack never delivers them to Corpus.

As a workaround, Corpus recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Corpus treats it identically to the slash form and replies in the same
thread.
...
Corpus falls back to a text prompt, the prompt instructs you to reply
...
Understanding how Corpus behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Corpus Agent what time is it?`). In channels, Corpus replies in a thread attached to that message. |
| **Threads** | If you @mention Corpus inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Corpus in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Corpus will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Corpus will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Corpus Agent`).

---

## Multi-Workspace Support

Corpus can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Corpus also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Corpus uses the correct workspace-specific client to respond.
...
Corpus supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Corpus Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Corpus Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Corpus now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Corpus Agent`)
...
- Audit who has access to your Corpus config directory
