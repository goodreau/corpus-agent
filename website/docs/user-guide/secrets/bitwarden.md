...
2. Corpus stores that single token in `~/.hermes/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `hermes` (or the gateway, or a cron job) starts, after `~/.hermes/.env` has loaded, Corpus calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Corpus **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Corpus process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.
...
2. Create or pick a **Project** (e.g. "Corpus keys").
...
4. **Machine accounts → New machine account → My Corpus machine** → **Projects** tab → grant Read access to your project.
...
Bitwarden never blocks Corpus startup. If anything goes wrong, you'll see a one-line warning in stderr and Corpus continues with whatever credentials `.env` already had:
...
- Corpus will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Corpus does not auto-upgrade `bws` to "latest" because upstream release shapes can change.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Corpus installations.
