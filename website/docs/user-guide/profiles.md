...
Run multiple independent Corpus agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Corpus home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Corpus state.
...
That's it. `coder` is now its own Corpus profile with its own config, memory, and state.
...
- A **profile** gives Corpus its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Corpus was launched from", not "the profile directory".
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.hermes/profiles/coder` before launching hermes. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, Corpus state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
...
- `HERMES_HOME` is the profile boundary. It controls Corpus config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Corpus
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Corpus keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Corpus launches tool
...
Corpus also exposes `HERMES_REAL_HOME` to subprocesses so scripts can still find
