---
sidebar_position: 7
title: "Use SOUL.md with Corpus"
description: "How to use SOUL.md to shape Corpus Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Corpus

`SOUL.md` is the **primary identity** for your Corpus instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Corpus to feel like the same assistant every time you talk to it — or if you want to replace the Corpus persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Corpus should be
- what Corpus should avoid stylistically
- how Corpus should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Corpus is and how Corpus speaks
...
Corpus now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run Corpus with a custom home directory, it becomes:
...
Corpus automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Corpus does not overwrite it
- if the file exists but is empty, Corpus adds nothing from it to the prompt

## How Corpus uses it

When Corpus starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Corpus feels.
...
Corpus already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Corpus should sound.

# Avoid
What Corpus should not do.

# Defaults
How Corpus should behave when ambiguity appears.
```
...
Then restart Corpus or start a new session.
...
### I edited SOUL.md but Corpus still sounds the same
...
### Corpus is ignoring parts of my SOUL.md
