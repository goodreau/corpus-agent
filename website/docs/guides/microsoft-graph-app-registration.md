...
4. Fill in:
   - **Name:** `Corpus Teams Meeting Pipeline` (or any name you'll recognize).
...
```powershell
# Create a policy scoped to the Corpus app
New-CsApplicationAccessPolicy `
  -Identity "Corpus-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Corpus meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Corpus-Meeting-Pipeline-Policy" `
...
Corpus ships a Graph auth smoke-test. From your Corpus install:
