...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Corpus Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`hermes claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Corpus. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| What | OpenClaw source | Corpus destination | Notes |
|------|----------------|-------------------|-------|
...
| Source | OpenClaw location | Corpus destination |
|--------|------------------|-------------------|
...
Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Corpus skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Corpus destination | Notes |
|------|---------------------|-------------------|-------|
...
| What | OpenClaw config path | Corpus config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Corpus config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Corpus field | Notes |
|----------------|-------------|-------|
...
| Platform | OpenClaw config path | Corpus `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Corpus path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Corpus equivalent)

These are saved to `~/.hermes/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Corpus |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Corpus has built-in tool instructions |
...
| Multi-agent list | `archive/agents-list.json` | Use Corpus profiles |
