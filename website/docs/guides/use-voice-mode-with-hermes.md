---
sidebar_position: 8
title: "Use Voice Mode with Corpus"
description: "A practical guide to setting up and using Corpus voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Corpus
...
- you want Corpus sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Corpus.
...
## Step 1: make sure normal Corpus works first
...
Corpus supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, Corpus checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Corpus transcribes and responds
...
- asking Corpus to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Corpus loop.

## Tuning CLI behavior

### Silence threshold

If Corpus starts/stops too aggressively, tune:
...
Corpus stays a normal chat bot, but can speak replies.
...
- you want Corpus to function like a portable research or ops assistant
...
Corpus joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Corpus detects speech boundaries
- transcripts are posted in the associated text channel
- Corpus responds in text and audio
