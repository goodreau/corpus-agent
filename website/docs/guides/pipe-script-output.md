...
messaging platform Corpus is already configured for. Think of it as a
...
Any platform Corpus ships adapters for works as a target:
...
When stdin is a TTY (no pipe), Corpus does **not** wait for input — you'll
...
Because `hermes send` reuses your Corpus config, the same script works on
any host where Corpus is installed — no need to export bot tokens into
...
| Approach | Multi-platform | Reuses Corpus creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
