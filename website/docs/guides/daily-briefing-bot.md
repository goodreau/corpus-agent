...
2. **Corpus spins up** a fresh agent session with your prompt
...
Corpus will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Corpus Agent
```
...
Before creating cron jobs, ensure Corpus has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Corpus what you want:
...
Corpus will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Corpus to delegate each topic to a sub-agent:
...
Corpus will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
