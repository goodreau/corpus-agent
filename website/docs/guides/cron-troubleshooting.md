...
description: "Diagnose and fix common Corpus cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
The `script` path must be an absolute path (or relative to the Corpus config directory). Verify:
```bash
...
hermes logs                         # View recent Corpus logs
