...
description: "Authoritative reference for Corpus built-in tools, grouped by toolset"
---

# Built-in Tools Reference

This page documents Corpus' built-in tools, grouped by toolset. Availability varies by platform, credentials, and enabled toolsets.
...
:::tip MCP Tools
In addition to built-in tools, Corpus can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp_<server>_` (e.g., `mcp_github_create_issue` for the `github` MCP server). See [MCP Integration](/user-guide/features/mcp) for configuration.
:::
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `execute_code` | Run a Python script that can call Corpus tools programmatically. Use this when you need 3+ tool calls with processing logic between them, need to filter/reduce large tool outputs before they enter your context, need conditional branching (… | — |
...
| `read_terminal` | Read what's currently shown in the in-app terminal pane of the Corpus desktop GUI (the embedded shell beside this chat). Desktop-app only. | — |
