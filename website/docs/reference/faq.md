...
description: "Frequently asked questions and solutions to common issues with Corpus Agent"
---
...
### What LLM providers work with Corpus?

Corpus Agent works with any OpenAI-compatible API. Supported providers include:
...
### I run Corpus in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Corpus inside WSL2
...
- let Corpus use the resulting MCP browser tools

This is more reliable than trying to force Corpus core browser transport to attach directly across the WSL2/Windows boundary.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Corpus Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.hermes/`.
...
# Context length: 64000   ← Corpus minimum; set this to match your server's actual context window
```
...
Corpus persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Corpus — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Corpus auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Corpus Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Corpus Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Corpus programmatically:
...
**Cause:** Corpus requires Python 3.11 or newer.
...
**Cause:** Corpus builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Corpus runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Corpus auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.hermes/config.yaml`:
...
```bash
# Exit the Corpus chat session first (Ctrl+C or /quit)
...
**Cause:** The conversation has grown too long for the model's context window, or Corpus detected the wrong context length for your model.
...
If this happens on the first long conversation, Corpus may have the wrong context length for your model. Check what it detected:
...
**Cause:** Corpus detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Corpus never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
# Restart Corpus or reload MCP after config changes
hermes chat
```
...
:::warning
If an MCP server crashes mid-request, Corpus will report a timeout. Check the server's own logs (not just Corpus logs) to diagnose the root cause.
:::
...
**Solution: Delegation config.** Corpus can route subagents to a different model automatically. Set this in `~/.hermes/config.yaml`:
...
Now when you tell Corpus "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Corpus do this?

**Current limitation:** Corpus profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Corpus reasoning, and tool call details in Telegram instead of just the final output.
...
**Current behavior:** Corpus creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Corpus to another machine
...
1. Install Corpus Agent on the new machine:
   ```bash
   curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
   ```
...
:::tip
`hermes backup` produces a consistent snapshot even while Corpus is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Corpus installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Corpus-specific issue — it's a shell config permissions problem.
...
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Corpus version (`hermes --version`), and the full error message
