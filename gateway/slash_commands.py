...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Corpus commands.
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Corpus Agent version."""
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Corpus default runtime
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Corpus Agent to the latest version.

        Spawns ``corpus update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``corpus update`` may trigger. Marker
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Corpus Agent')}"
